    ...
    for i in tqdm(range(len(df)), desc="Passages"):
        ...
        try:
            ...
            with open(output_csv, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    f'fail_{name}', 'fail', 'fail', 'fail', 'fail'
                    ])
            time.sleep(5)
            continue
...
def main():
    ...
    # Perform zeroshot
    process(
        instruction, "qwen_bi_zeroshot_output_lg.csv", df, model, tokenizer
    )

    # Uncomment the following three lines to perform oneshot
#    example = get_example(1)
#    prompt = instruction + example
#    process(prompt, "qwen_bi_oneshot_output_test.csv", df, model, tokenizer)

    # Uncomment the following three lines to perform threeshot
#    examples = get_example(3)
#    prompt = instruction + examples
#    process(prompt, "qwen_bi_3-shot_output_test.csv", df, model, tokenizer)
