...
def get_instruction():
    ...
    You are going to analyze and identify the text's degree of the narrativity structural elements 'agency', 'event sequencing' and 'world making' on a Five Point Likert scale: 1 (strongly disagree), 2 (somewhat disagree), 3 (unsure), 4 (somewhat agree), 5 (strongly agree).
    Then you will also decide whether the text contains a story or not.

    ### Instructions
    Think step by step.
    You are going to rate the text in terms of how strongly you agree with the following statements:
    ...
    World making: “This passage describes a location or a place at which something occurs.”
    Your rating will be done according to the Five Point Likert Scale.
    Decide if the passage contains a story.

    ### Output
    Respond strictly in the following structure by using the corresponding numerical degree per element (stricly one number per element ranging from 1 to 5) and 0 (no) or 1 (yes) for story, without further explanation:
    “Agency: #value
    Event sequencing: #value
    World making: #value
    Story: #value”
...
def get_example(n):
    ...
    examples_text = "\n\n".join(all_examples[:n])
    return f'''\n### Examples
    Follow the following example{'s' if n > 1 else ''}, they contain a passage and its corresponding correct degrees of the elements:
