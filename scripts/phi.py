...
def main():
    ...
    # Perform zeroshot
    process(
        instruction, "phi_zeroshot_output_test.csv",
    ...
    process(prompt, "phi_oneshot_output_test.csv", df, pipe, generation_args)

    # Perform threeshot
    examples = get_example(3)
    prompt = instruction + examples
    process(prompt, "phi_3-shot_output_test.csv", df, pipe, generation_args)
