...
def main():
    ...
    # Perform zeroshot
    process(
        instruction, "llama_zeroshot_output_test.csv",
        df, pipe, generation_args
    )

    # Perform oneshot
    example = get_example(1)
    prompt = instruction + example
    process(prompt, "llama_oneshot_output_test.csv", df, pipe, generation_args)

    # Perform threeshot
    examples = get_example(3)
    prompt = instruction + examples
    process(prompt, "llama_3-shot_output_test.csv", df, pipe, generation_args)
